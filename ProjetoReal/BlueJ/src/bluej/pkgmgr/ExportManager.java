...
final class ExportManager {
	...
	public void export() {
		...
		String fileName = FileUtility.getFileName(frame, specifyJar, createJarText, null, false);
		if (fileName == null)
			return;

		String sourceDir = frame.getProject().getProjectDir().getPath();

		createJar(fileName, sourceDir, dialog.getMainClass(), dialog.getSelectedLibs(), dialog.includeSource(),
				dialog.includePkgFiles());
	}

	/**
	 * Export this project to a jar file.
	 */
	private void createJar(String fileName, String sourceDir, String mainClass, List<File> userLibs,
			boolean includeSource, boolean includePkgFiles) {
	 ...
	 * file)
	 */
	private void writeDirToJar(File sourceDir, String pathPrefix, JarOutputStream jStream, boolean includeSource,
			boolean includePkg, File outputFile) throws IOException {
				...
				for (int i = 0; i < dir.length; i++) {
					if (dir[i].isDirectory()) {
						if (!skipDir(dir[i], includePkg)) {
							writeDirToJar(dir[i], pathPrefix + dir[i].getName() + "/", jStream, includeSource,
									includePkg, outputFile);
						}
					} else {
						// check against a list of file we don't want to export
						// and
						// also
						// check that we don't try to export the jar file we are
						// writing
						// (hangs the machine)
